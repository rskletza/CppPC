...
Vector * vector__new()
{
	Vector * v = (Vector*) malloc(sizeof(int)*2+sizeof(VectorValue *)); 
	v->_count = 0;
	v->_max = 0;
	v->_values = NULL;
	return v;
...
void vector__push_back(Vector * _this, VectorValue val)
{
	...
	{
		_this->_values = (VectorValue *) realloc(_this->_values, sizeof(VectorValue)*_this->_max+CHUNK);
		_this->_max = _this->_max + CHUNK;
	}
...
VectorValue vector__pop_back(Vector * _this)
{
	...
	*(vector__end(_this)) = 0;
	if (_this->_max - _this->_count > CHUNK)
	_this->_values = (VectorValue *) realloc(_this->_values, sizeof(VectorValue)*_this->_max - CHUNK);
	return val;
}
